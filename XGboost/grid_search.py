...
TARGET_COL = "cases_per_10000"            # or "cases" to model counts directly
...
def first_scored_h(max_horizon, window=TARGET_WINDOW):
    """First horizon that lands inside the scored window (e.g. 16 for max_horizon=67)."""
...
def _mmwr_epiweek1_start(year):
    """Sunday that starts MMWR week 1 = the Sunday of the week containing Jan 4."""
    jan4 = date(year, 1, 4)
    return jan4 - timedelta(days=(jan4.weekday() + 1) % 7)


def _weeks_in_epiyear(year):
    """MMWR weeks in `year` (52 or 53), straight from the calendar -- no data needed."""
    return (_mmwr_epiweek1_start(year + 1) - _mmwr_epiweek1_start(year)).days // 7


def season_spec(dataset_id, gap_weeks=GAP_WEEKS):
    """(max_horizon, window) for a dataset from the epiweek calendar.

        window      = weeks_in_epiyear(season year)   # 52 or 53
        max_horizon = gap_weeks + window              # 67 or 68  (first_scored_h stays 16)
    """
    train, _ = data_factory(dataset_id)
    year = int(train[TIME_COL].max() // 100)
    window = _weeks_in_epiyear(year)
    return gap_weeks + window, window
...
def build_supervised(
    ...
    """Tall (X, y) like data_processing.make_dataset, with:
      - y         : the training target at t+h, in `target_col` units
      - keys frame: uf_code, origin_epiweek, h, target_date, population, cases
                    (population & cases are at the TARGET week, for count-space scoring)
    Feature columns come from the shared _build_features, so there is no skew vs
    make_forecast_frame.
    """
...
def _row_index(keys):
    """A MultiIndex identifying each row's (state, origin week, horizon)."""
    return pd.MultiIndex.from_frame(keys[["uf_code", "origin_epiweek", "h"]])



def expanding_folds(origin_ord, n_folds, val_size, embargo):
    """Yield (train_idx, val_idx) integer arrays over rows.

    origin_ord : per-row ordinal week index of the *origin* (0..W-1, contiguous).
    val_size   : width of each validation window, in weeks.
    embargo    : gap (weeks) between train end and val start; set = max_horizon so a
                 training row's target can't land inside the validation window.
    """
...
def make_submission(dataset_id, max_lag, params, max_horizon=None, model_out=None, device=None):

    if max_horizon is None:
        max_horizon, _ = season_spec(dataset_id)
...
def score_against_target(dataset_id, submission):
    ...
    _, window = season_spec(dataset_id)
    if test["date"].nunique() < window:
        return None
