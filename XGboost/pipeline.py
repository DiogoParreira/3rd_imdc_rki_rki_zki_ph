...
from grid_search import (
    ...
    first_scored_h, season_spec, GAP_WEEKS, _base_xgb, _postprocess, _to_counts, wis,
)
...
def _resolve_horizon(dataset_id, cli_max_horizon):

    if cli_max_horizon is None:
        return season_spec(dataset_id)
    return cli_max_horizon, cli_max_horizon - GAP_WEEKS
...
def cmd_prep(args):
    ...
    for ds in args.datasets:
        t = time.time()
        mh, window = _resolve_horizon(ds, args.max_horizon)
        X, y, keys = build_supervised(ds, args.max_lag, mh)
        order = {w: i for i, w in enumerate(sorted(keys["origin_epiweek"].unique()))}
        oord = keys["origin_epiweek"].map(order).to_numpy()
        folds = backtest_folds(oord, keys["h"].to_numpy(), mh,
        ...
        np.save(os.path.join(d, "h.npy"), keys["h"].to_numpy(np.int16))   # for window-only scoring
        ...
        with open(os.path.join(d, "meta.json"), "w") as f:
            json.dump({"dataset": ds, "build_max_lag": args.max_lag,
                       "max_horizon": mh, "window": window, "n_folds": len(folds),
                       "feature_cols": list(X.columns)}, f)
        print(f"ds{ds}: X{X.shape} {len(folds)} folds  max_horizon={mh} window={window} "
              f"({time.time() - t:.1f}s) -> {d}", flush=True)
...
def cmd_search(args):
    ...
    min_h = first_scored_h(meta["max_horizon"], meta.get("window", VAL_SIZE))
...
def cmd_baseline(args):
    ...
    for ds in ds_list:
        ...
        min_h = first_scored_h(mh, window)
        order = {w: i for i, w in enumerate(sorted(keys["origin_epiweek"].unique()))}
        oord = keys["origin_epiweek"].map(order).to_numpy()
        folds = backtest_folds(oord, h_arr, mh, window=window, n_folds=N_FOLDS)
        ...
        sub = make_submission(ds, args.max_lag, params, mh,
        ...
        with open(os.path.join(out_dir, f"ds{ds}_baseline.json"), "w") as f:
            json.dump({"dataset": ds, "max_lag": args.max_lag, "max_horizon": mh, "params": params,
...
def cmd_finalize(args):
    ...
    for ds in args.datasets:
        ...
        mh, _ = _resolve_horizon(ds, args.max_horizon)
        print(f"ds{ds}: best WIS={best['wis']:.3f} max_lag={int(best['max_lag'])} "
              f"n_est={params['n_estimators']} max_horizon={mh} ({len(df)} configs) -> refitting on all rows",
              flush=True)

        sub = make_submission(ds, int(best["max_lag"]), params, mh,
        ...
        with open(os.path.join(final_dir, f"ds{ds}_best_config.json"), "w") as f:
            json.dump({"max_lag": int(best["max_lag"]), "max_horizon": mh, "params": params,
...
def main():
    ...
    def common(p):
        ...
        p.add_argument("--max-horizon", type=int, default=None, dest="max_horizon",
                       help="forecast horizon; default = epiweek-aware per dataset (67, or 68 for a 53-week season)")
